...
module.exports = class enviarRequest {
    ...
    async _getInformacoesLCU(urlAlvo) {
        ...
        this.requestAPI.get(corpoRequest, retornoRequest);
    ...
    _putInformacoesLCU(tipo, urlAlvo, putInformacao, uniqueRequestRule) {
        ...
        this.requestAPI.put(scopeRequestPut, function (error, response, body) {
            console.log('A solicitacao [' + tipo + '] foi enviada. O retorno foi de: ' + response.statusCode)
        })
    ...
    _postInformacoesLCU(urlAlvo) {
        ...
        this.requestAPI.post(JSON.parse(scopeRequestPost))
    }
}